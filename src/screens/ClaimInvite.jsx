// Invite-claim: the coach pre-creates the student row and sends an invite. The
// emailed link establishes a session (and handle_new_user has already auto-linked
// the roster row by email). Here the student just sets their first password.
import { useState } from 'react'
import { Navigate, Link } from 'react-router-dom'
import { useAuth } from '../auth/useAuth'

export default function ClaimInvite() {
  const { session, loading, user, setPassword } = useAuth()
  ...
  // Wait for the invite link's session to settle.
  ...
  // Students always land on their home after claiming.
  ...
  return (
    <Centered title="Welcome to 55TC">
      <p className="mt-1 text-sm text-ink/60">
        {user?.email ? `Setting up ${user.email}` : 'Set your password to get started.'}
      </p>

      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        <PasswordField label="Create password" value={password} onChange={setPwd} />
