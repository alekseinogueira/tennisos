// The full session-feedback dashboard, presentational only — extracted from
// SessionDetail (Fase F1, Etapa 3) so the coach's review screen can render the
// EXACT same thing the student will see. Screens own the data fetching and the
// chrome around it (back button, wrappers); this renders header → footer.
...
/** compareHref: footer "Compare with another session →" target, or null to hide
 *  it (the coach review has no student compare context). */
export default function SessionFeedbackView({ feedback, studentName, videos, compareHref }) {
  const {
    title,
    body,
    lesson_date,
