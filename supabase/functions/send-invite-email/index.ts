...
//
// Sends the 55TC portal invite email via Resend.
// POST body: { student_name, student_email, invite_link }
...
const SUBJECT = "Your 55TC portal is ready.";
...
function buildEmailHtml(studentName: string, inviteLink: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>55TC — You're in.</title>
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .logo-wrap { position: relative; height: 24px; margin-bottom: 28px; }
  .logo-wrap svg { height: 24px; width: auto; display: block; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.2)); }
  ...
  .body { padding: 40px 40px 32px; background-color: #F5EDE0; }
  ...
  .cta-note { margin-top: 14px; font-size: 12px; color: #1C3526; opacity: 0.4; line-height: 1.5; }
...
<div class="wrapper">
  <div class="header">
    <div class="logo-wrap">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 920 218" height="24" width="auto" aria-label="55TC OS">
        <defs><filter id="h-shadow" x="-10%" y="-10%" width="130%" height="140%"><feDropShadow dx="0" dy="4" stdDeviation="6" flood-color="#000000" flood-opacity="0.22"/></filter></defs>
        <g filter="url(#h-shadow)" transform="translate(-149.8, 610.864) scale(0.1, -0.1)" fill="#F5EDE0" stroke="none">
        ...
        <text x="734" y="178" font-family="'Bebas Neue','Arial Black',Impact,sans-serif" font-size="195" font-weight="400" fill="#F5EDE0" fill-opacity="0.38" filter="url(#h-shadow)" letter-spacing="-2">OS</text>
      </svg>
    </div>
    <div class="header-headline">YOU'RE<br>ON THE<br>COURT.</div>
    <div class="header-sub">55TC · Vancouver</div>
  </div>
  <div class="body">
    <p>Hey <span class="name-highlight">${studentName}</span>,</p>
    <p>Great to have you here. Tennis is an excellent choice and I'm sure you're going to do really well. You can count on me for that. I'm excited to get started and help you develop your game.</p>
    <p>I set up your player profile so you can track your sessions, watch your footage, and see your progress over time. Takes two minutes to activate.</p>
    <p style="margin-top: 20px; font-weight: 500;">— Aleksei</p>
  </div>
  <div class="cta-block">
    <div class="cta-label">Next step</div>
    <a href="${inviteLink}" class="cta-button">ACTIVATE YOUR ACCOUNT →</a>
    <div class="cta-note">Link expires in 7 days. Questions? Just reply to this email.</div>
  </div>
  <div class="footer">
...
Deno.serve(async (req) => {
  ...
  const { student_name, student_email, invite_link } = payload;
  if (!student_name || !student_email || !invite_link) {
    return json(
      { error: "student_name, student_email and invite_link are required." },
      400,
    );
  }
  ...
  const res = await fetch(RESEND_ENDPOINT, {
    ...
    body: JSON.stringify({
      ...
      to: [student_email],
      subject: SUBJECT,
      html: buildEmailHtml(student_name, invite_link),
    }),
  });
